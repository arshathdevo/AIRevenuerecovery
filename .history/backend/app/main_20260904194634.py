...
def create_payment(payment_data: PaymentCreate):
    ...
    if obligation.recovery_attempted:
        obligation.status = RevenueStatus.RECOVERED
    else:
...
@app.post("/revenue-obligations/{revenue_id}/start-recovery")
def start_recovery(revenue_id: str):

    if revenue_id not in revenue_obligations:
        raise HTTPException(
            status_code=404,
            detail="Revenue obligation not found"
        )

    obligation = revenue_obligations[revenue_id]

    if obligation.status != RevenueStatus.AT_RISK:
        raise HTTPException(
            status_code=400,
            detail="Recovery can only start for AT_RISK revenue"
        )

    obligation.recovery_attempted = True
    obligation.status = RevenueStatus.RECOVERY_IN_PROGRESS

    return {
        "message": "Recovery started",
        "revenue_id": revenue_id,
        "recovery_attempted": obligation.recovery_attempted,
